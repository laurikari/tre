		    ...
		    int eflags, int *match_end_ofs)
{
  ...
  while (/*CONSTCOND*/(void)1,1)
	...
	tre_tnfa_approx_reach_t *ringbuffer[TRE_EXPRESSION_RING_BUFFER_SIZE_IN_BYTES];
	...
	while (deque_end != deque_start)
	  {
		...
		deque_start++;
		if (deque_start >= (ringbuffer + TRE_EXPRESSION_RING_BUFFER_SIZE_IN_BYTES))
		...
		*deque_end = &reach_next[dest_id];
		deque_end++;
		if (deque_end >= (ringbuffer + TRE_EXPRESSION_RING_BUFFER_SIZE_IN_BYTES))
