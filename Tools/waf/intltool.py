...
def configure(conf):
	# TBR: modified to check only for required features if conf.env.intltool_features has been set.
	reqd_features = (conf.env.intltool_features if "intltool_features" in conf.env else ["intltool_po","intltool_in"])
	if "intltool_po" in reqd_features:
		conf.find_msgfmt()
	if "intltool_in" in reqd_features:
